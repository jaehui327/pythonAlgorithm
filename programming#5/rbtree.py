...
class Dict:
    ...
    def insert (self, v):
        ...
        if x.left.color and x.right.color: self.split(x, p, g, gg, v)
    ...
    def split(self, x, p, g, gg, v):
        ...
        if p.color:
            g.color = red
            if (g.key > v) != (p.key > v): p=self.rotate(v, g)
            x = self.rotate(v, gg)
            x.color = black
    ...
    def check(self, search_key):
        x = self.head.right
        parent = self.head.right
        while x != self.z:
            if x.key == search_key:
                return (parent.key, x.color)
            if x.key > search_key:
                parent = x
                x = x.left
            else:
...
for i in range(1, N + 1):
    result=d.check(i)
    if result[1]: color = 'black'
    else: color = 'red'
    print('key: ', i, ', parents: ', result[0], ', color: ', color)
